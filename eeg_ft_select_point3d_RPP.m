function [selected] = eeg_ft_select_point3d_RPP(bnd, varargin)
...
screen_resol = get(0, 'MonitorPositions');%(1920 x 1080) Ancho x Alto (ó:    get(0,'ScreenSize')   )
% Esto es plotear el modelo
close all; 
set(0,'units','pixels');% Todo son pixeles...!!!
x=ft_plot_mesh(bnd);
view(180,90);
load('RPP_file');       % [NAS] [angulo] 
rotate(x,[0 1 0],angulo-87);
%% 
img=getframe;
img=img.cdata;
img = img;
...
%% 
set(groot,'PointerLocation',[RPA(2),a-RPA(1)]);
...
set(groot,'PointerLocation',[RPA(1),a-RPA(2)]);
%% *****************************************************************************************************************************
%while ~done
